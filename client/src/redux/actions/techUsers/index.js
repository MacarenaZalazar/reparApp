...
import { GET_TECH_USERS_ALL, GET_TECH_USERS_BY_ID, GET_TECH_USERS_BY_JOB_ZONE } from "./constantsTechUsers";
...
export function getTechUsersByJobAndZone(jobType, zone){
  console.log('llegué a la action')
    return async function (dispatch) {
      try {
        let techUsers = await axios.get(`${TECH_USERS_URL}?jobTypes=${jobType}&workZones=${zone}`);
        return dispatch({
